...
class Neigh_List(torch.nn.Module):
    ...
    def forward(self,period_table,coordinates,cell):
        ...
        num_repeats = torch.ceil(torch.min(self.cutoff/torch.abs(cell),dim=0)[0])
        # the number of periodic image in each direction
        num_repeats = period_table*num_repeats.detach()
        r1 = torch.arange(-num_repeats[0], num_repeats[0] + 1, device=coordinates.device)
        r2 = torch.arange(-num_repeats[1], num_repeats[1] + 1, device=coordinates.device)
        r3 = torch.arange(-num_repeats[2], num_repeats[2] + 1, device=coordinates.device)
