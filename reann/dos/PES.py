...
class PES(torch.nn.Module):
    ...
    def forward(self,period_table,cart,cell,species,mass,dos_ene):
        ...
        output = torch.sum(self.nnmod(density,species))+dos_ene
        input_dos = torch.cat((density.view(1,cart.shape[0],-1).expand(dos_ene.shape[0],-1,-1),output.view(-1,1,1).expand(-1,cart.shape[0],-1)),dim=2).reshape(-1,density.shape[1]+1)
        atom_dos=self.nnmod1(input_dos,species.view(1,-1).expand(dos_ene.shape[0],-1).reshape(-1))
        dos = torch.sum(atom_dos.view(dos_ene.shape[0],-1),dim=1)
        return dos/float(cart.shape[0])
