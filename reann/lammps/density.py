...
class GetDensity(torch.nn.Module):
    ...
    def forward(self,cart,atom_index,local_species,neigh_species):
        ...
        orb_coeff=self.params.index_select(0,neigh_species).view(-1,self.nipsin.shape[0],self.rs.shape[1]).\
        index_select(1,self.index_para)
        worbital=torch.einsum("ijk,jkm,ijm ->ijm", orbital,expandpara,orb_coeff)
        ...
        density=torch.zeros((nlocal,self.nipsin.shape[0],self.rs.shape[1]),dtype=orb_coeff.dtype,device=orb_coeff.device)
