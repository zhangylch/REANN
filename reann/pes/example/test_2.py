...
badfile="badpoint"+count
f2=open(badfile,"w")
rest_file="configuration_"+count
ori_file="oridata_"+count
f3=open(rest_file,"w")
fname1="error"+count
f4=open(fname1,"w")
with open(ori_file,'r') as f1:
    while True:
        string=f1.readline()
        if not string: break
        string=f1.readline()
        ...
        f4.write("{} \n".format((energy-abene).detach().cpu().numpy()[0]))
        f4.flush()
        if torch.abs(energy-abene)<error:
           ...
           npoint+=1
           npoint1+=1
           if torch.abs(energy-abene)>0.002:
...
f3.close()
f4.close()
directive="mv "+rest_file+" "+ori_file
os.system(directive)
